...
with col1:
    core_pce = st.number_input("Core PCE YoY (%) / เงินเฟ้อพื้นฐาน PCE", value=2.0, step=0.1, format="%.2f")
    core_cpi = st.number_input("Core CPI YoY (%) / เงินเฟ้อพื้นฐาน CPI", value=2.2, step=0.1, format="%.2f")
    ten_y = st.number_input("US 10Y Yield (%) / บอนด์ยีลด์ 10 ปี", value=4.0, step=0.1, format="%.2f")
    fed_rate = st.number_input("Fed Funds Rate (%) / อัตราดอกเบี้ยนโยบายสหรัฐ", value=5.25, step=0.25, format="%.2f")
with col2:
    pmi = st.number_input("PMI (Global/ISM) / ดัชนี PMI", value=50.0, step=0.1, format="%.1f")
    unemp = st.number_input("US Unemployment Rate (%) / การว่างงาน", value=3.8, step=0.1, format="%.2f")
    dxy = st.number_input("US Dollar Index (DXY) / ดัชนีดอลลาร์", value=103.0, step=0.1, format="%.1f")
    debt_gdp = st.number_input("US Debt-to-GDP (%) / หนี้สาธารณะต่อ GDP", value=120.0, step=1.0, format="%.0f")
with col3:
    m2 = st.number_input("M2 YoY Growth (%) / การเติบโตของ M2", value=2.0, step=0.5, format="%.1f")
    repo = st.number_input("Overnight Repo Rate (%) / ดอกเบี้ยรีโป", value=5.0, step=0.1, format="%.2f")
    margin = st.number_input("Margin Debt (USD bn) / ยอดมาร์จิ้น (พันล้านดอลลาร์)", value=900.0, step=50.0, format="%.0f")
with col4:
    gold = st.number_input("Gold (USD/oz) / ราคาทอง", value=2500.0, step=1.0, format="%.0f")
    spx = st.number_input("S&P 500 Index / ดัชนี S&P 500", value=5600.0, step=1.0, format="%.0f")
    btc = st.number_input("Bitcoin (USD) / ราคา BTC", value=70000.0, step=100.0, format="%.0f")
